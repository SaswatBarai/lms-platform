...
╚══════════════════════════════════════════════════════════════╝
    ...
    static async sendStaffWelcomeEmail(name: string, email: string, tempPassword: string, loginUrl: string, retries = 2) {
        // Check if we should use console mode
        if (process.env.EMAIL_MODE === 'console') {
            console.log(`
╔══════════════════════════════════════════════════════════════╗
║              📧 STAFF ACCOUNT CREATED (CONSOLE)              ║
╠══════════════════════════════════════════════════════════════╣
║  Name: ${name.substring(0, 50).padEnd(50)} ║
║  📧 Email: ${email.padEnd(48)} ║
║  🔑 Password: ${tempPassword.padEnd(44)} ║
║  🔗 Login URL: ${loginUrl.substring(0, 43).padEnd(43)} ║
║  💡 Mode: Console (Email disabled for development)          ║
╚══════════════════════════════════════════════════════════════╝
            `);
        ...
        if (!isVerified && env.NODE_ENV === 'development') {
            console.log(`
╔══════════════════════════════════════════════════════════════╗
║             📧 STAFF ACCOUNT (DEVELOPMENT FALLBACK)          ║
╠══════════════════════════════════════════════════════════════╣
║  Name: ${name.substring(0, 50).padEnd(50)} ║
║  📧 Email: ${email.padEnd(48)} ║
║  🔑 Password: ${tempPassword.padEnd(44)} ║
...
╚══════════════════════════════════════════════════════════════╝
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                if (attempt > 0) {
                    console.log(`[Notification] Retry attempt ${attempt}/${retries} for staff welcome email to ${email}`);
                    await new Promise(resolve => setTimeout(resolve, Math.pow(2, attempt) * 1000));
                }

                console.log(`[Notification] 📤 Attempting to send staff welcome email to ${email}`);
                
                const result = await transporter.sendMail({
                    from: `LMS Platform <${env.MAIL_USER}>`, 
                    to: email,
                    subject: `Welcome ${name}! Your Staff Account is Ready`,
                    html: htmlForStaffAccountCreated(name, tempPassword, loginUrl)
                });
                
                if (result.rejected.length > 0) {
                    console.error(`[Notification] ❌ Staff welcome email rejected for ${email}:`, result.rejected);
                    if (attempt === retries) {
                        this.logStaffPasswordToConsole(name, email, tempPassword, 'Email rejected by server');
                        return false;
                    }
                    continue;
                } else {
                    console.log(`[Notification] ✅ Staff welcome email sent successfully to ${email}`);
                    console.log(`[Notification] 📨 Message ID: ${result.messageId}`);
                    return true;
                }
            } catch (error: any) {
                console.error(`[Notification] ❌ Error sending staff welcome email to ${email}:`, error.message);
                
                if (attempt === retries) {
                    console.error(`[Notification] ⚠️  Failed to send staff welcome email after ${retries + 1} attempts`);
                    this.logStaffPasswordToConsole(name, email, tempPassword, error.message);
                    return false;
                }
            }
        }
    ...
    private static logStaffPasswordToConsole(name: string, email: string, tempPassword: string, reason: string) {
        console.log(`
╔══════════════════════════════════════════════════════════════╗
║                  🚨 STAFF PASSWORD FALLBACK                  ║
╠══════════════════════════════════════════════════════════════╣
║  Name: ${name.substring(0, 50).padEnd(50)} ║
║  📧 Email: ${email.padEnd(48)} ║
║  🔑 Password: ${tempPassword.padEnd(44)} ║
...
const htmlForStaffAccountCreated = (staffName: string, tempPassword: string, loginUrl: string) => `
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
